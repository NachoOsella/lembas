...
public class PreferenceService {
    ...
    private Order loadAndValidateOrder(Long orderId, User customer) {
        if (customer == null || customer.getRole() != Role.CUSTOMER) {
            throw new DomainException(
                    PaymentErrorCodes.ACCESS_DENIED,
        ...
        Order order = orderQuery
                ...
                .orElseThrow(() -> new DomainException(
                        PaymentErrorCodes.ORDER_NOT_FOUND, HttpStatus.NOT_FOUND, "Order not found"));
        if (order.getType() != OrderType.ONLINE) {
            throw new DomainException(
                    PaymentErrorCodes.ORDER_NOT_PAYABLE,
                    HttpStatus.CONFLICT,
                    "Only online orders support hosted checkout");
        }
        if (order.getStatus() != OrderStatus.PENDING_PAYMENT) {
            throw new DomainException(
                    PaymentErrorCodes.ORDER_NOT_PAYABLE, HttpStatus.CONFLICT, "Order is not in a payable state");
        }
        return order;
    }
    ...
    private CreatePreferenceResponse requestProviderPreference(Order order, Payment payment) {
        ...
        log.debug(
                "Preference created: id={} initPoint={} sandbox={} selected={}",
                result.preferenceId(),
                result.initPoint(),
    ...
    private void cancelOpenPayments(Order order) {
        List<Payment> open = paymentRepository.findByOrderIdAndProviderAndStatusInOrderByIdAsc(
                order.getId(), PaymentProvider.MERCADO_PAGO, List.of(PaymentStatus.PENDING, PaymentStatus.IN_PROCESS));
